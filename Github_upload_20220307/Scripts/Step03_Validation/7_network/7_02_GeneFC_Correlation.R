# -------------------------------
# -------------------------------
# Code to run Gene-FC correlation analyses in 7-network parcellation:
# 
...
## 2. Integration: Gene-FC correlation in 7-network parcellation
# ----------------
...
p.mat <- test1 + test2 
rownames(p.mat) <- plot.order.7[twohemi.net7.regions]
colnames(p.mat) <- plot.order.7[twohemi.net7.regions]
...
# Gene
diag(avg.mat) <- 0
pdf(file = paste(base.dir, '/Output/SupplementaryData/SupplementaryFigure/FigS2a_corrplot_443genes_net7.pdf', sep = ''),
    width = 10, height = 10)

corrplot(corr = as.matrix(avg.mat), type="upper", method = "color",  # diag=FALSE,
         col = col2(100), tl.pos = "lt", tl.cex = 1.5, tl.col = "black",
         col.lim = c(-0.75, 0.75))
corrplot(corr = as.matrix(avg.mat), add=TRUE, type="upper", method = "color", diag=FALSE, cl.pos = "n",
         ...
         addCoef.col = "black",  number.digits = 2, number.cex = 1.3)
corrplot(corr = as.matrix(avg.mat), add=TRUE, type="lower", method="color",diag=FALSE,tl.pos="n", cl.pos="n", 
         col = col2(100), 
         p.mat = as.matrix(p.mat), insig = "label_sig", sig.level = .05, pch.cex = 1.3, pch.col = "black")
dev.off()
diag(avg.mat) <- 1

#FC
diag(plot.func) <- 0 
pdf(file = paste(base.dir, "/Output/SupplementaryData/SupplementaryFigure/FigS2b_corrplot_fc_net7.pdf", sep=''),
    width = 10, height = 10)
...
write.csv(x=avg.mat, file=paste0(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet13_Gene_cere_Corr_net7.csv')) # same as generated in the cere script
write.csv(x=plot.func, file=paste0(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet14_FC_cere_Corr_net7.csv')) # same as generated in the cere script
# ----------------