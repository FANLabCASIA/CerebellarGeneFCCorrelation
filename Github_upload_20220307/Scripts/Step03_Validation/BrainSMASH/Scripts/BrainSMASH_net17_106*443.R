...
for ( donor in donor.nums[1:2]){ # donor = "9861"
  ...
  #Write out the 443
  gene.idxs <- which(rownames(all_data[['10021']]$cere_expr) %in% unique(cere.n6.sig.genes)) # no difference between 7 or 17, we just use the idx to indicate the gene
  mat <- all.expr[gene.idxs, ]
} 
write.csv(x = Lab, file = paste0(base.dir, '/Scripts/Step03_Validation/BrainSMASH/Results/17_network/Annotation.csv'))
write.csv(x = mat, file = paste0(base.dir, '/Scripts/Step03_Validation/BrainSMASH/Results/17_network/Mat.csv'))
# ----------------
...
# ----------------
# setwd("/n02dat01/users/ypwang/AHBA/BrainSMASH/Net_17/Original/Permutationmap_10000/10021/")
# filelist <- list.files(pattern=".*.csv")
# m <- length(filelist)
# m
# datalist[[donor]] <- lapply(filelist[1:10000], function(x) read.csv(x,header=F,stringsAsFactors=F, fileEncoding = 'utf-8'))

# Read in the data from "SA_control_permutation.py"
datalist <- NULL
for ( donor in donor.nums[1:2] ){
  # setwd(paste0('/n02dat01/users/lchai/BrainsMASH_yp/Net_17/output_dir_1/Permutationmap_10000/', donor, '/'))
  setwd(paste0('/n02dat01/users/lchai/BrainsMASH_yp/Net_17/output_dir/Permutationmap_10000/', donor, '/'))
 
  filelist <- list.files(pattern=".*.csv")
  m <- length(filelist)
  m
  datalist[[donor]] <- lapply(filelist, function(x) read.csv(x,header=F,stringsAsFactors=F, fileEncoding = 'utf-8'))
}
...
View(R_random[,R_random[1,]>0.4474744])
length(R_random[R_random[1,]>0.4787607])/length(R_random[1,])  # 0.015,   0.0075
length(R_random[R_random[1,]>0.4904169])/length(R_random[1,])  # 0.0192,  0.0096
length(R_random[R_random[1,]>0.4474744])/length(R_random[1,])  # 0.0254,  0.0127

sum(R_random[1,]>0.4787607)/length(R_random[1,])     # 0.0075
sum(R_random[1,]>0.4904169)/length(R_random[1,])     # 0.0337
sum(R_random[1,]>0.4474744)/length(R_random[1,])     # 0.0471
# ----------------


## 4. Plot the BrainSMASH results
# ----------------
L_cur <- R_random[R_random[1,]>0.4787607]
p_cur <- length(L_cur)/length(R_random[1,])   
plot.perm <- data.frame(count = c(1:length(R_random[1,])), R = as.numeric(R_random[1,])) 

pdf(file =  paste0(base.dir, "/Output/Figure/Fig2c_BrainSMASH.pdf", sep=''),
    width = 9, height = 5)
par(mfcol=c(3,1))
