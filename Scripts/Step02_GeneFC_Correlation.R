...
t.write.genes <- t(write.genes)
write.me      <- rbind(region.names, rbind(write.nums, t.write.genes))
write.csv(x=write.me, file=paste0(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet7_Gene_cort_Network.csv'), row.names=FALSE)

# Write the genes that passed the sig threshold
cort.n6.sig.genes  <- unique(cort.n6.sig.genes )
write.table(x=cort.n6.sig.genes , file=paste0(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet8_Gene_cort_GeneList.csv'), row.names=FALSE, col.names=FALSE, quote=TRUE)
...
write.csv(x=out.matrix, file=paste0(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet6_Gene_cort_DiffExpr.csv')) 
# ----------------
...
write.genes <- vector()
write.nums  <- NULL
for (buckner in use.cere.networks){ # buckner <- 'SomMot'
...
base      <- paste(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet9_Gene_cortcere_Network.csv', sep = '')
...
write.csv(x = overlap.genes, 
          file = paste(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet10_Gene_cortcere_GeneList.csv', sep = ''), 
          row.names=FALSE)
...
pdf(file = paste(base.dir, '/Output/Figure/Fig2a_corrplot_443genes.pdf', sep = ''),
    width = 10, height = 10)
...
diag(avg.mat) <- 1
write.csv(file = paste(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet11_Gene_cere_Corr_net17.csv', sep=''), x = avg.mat)

# The FC matrix generated from HCP data, includes three group: 1018 hcp subjects, 211 hcp unrelated subjects and 296 hcpa subjects
cere2cere.fcmri.17  <- read.csv(paste0(base.dir,'/Data/FC/cere17-cor17-all_fisher/gsr_cerecere.csv'), header = FALSE)
# cere2cere.fcmri.17 <- read.csv(paste0(base.dir,'/Data/FC/results_unrelated_17x17.csv'), header = FALSE)
# cere2cere.fcmri.17 <- read.csv(paste0(base.dir,'/Data/FC/17x17_296.CSV'), header = FALSE)
...
diag(plot.func) <- 0 
pdf(file = paste(base.dir, "/Output/Figure/Fig2b_corrplot_fc_17.pdf", sep=''),
    width = 10, height = 10)
...
diag(plot.func) <- 1
write.csv(file = paste(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet12_FC_cere_Corr_net17.csv', sep=''), x = plot.func)
# write.csv(file = paste(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet17_FC_cere_net17_unrelated.csv', sep=''), x = plot.func)
# write.csv(file = paste(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet18_FC_cere_net17_HCPA.csv', sep=''), x = plot.func)
...
cor.test(func.1d.arr, mrna.1d.arr)  # 0.4787607, 0.4904169, 0.4474744
# ----------------
...
write.csv(file = paste(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet22_GCI+_List_n246.csv', sep=''), x = rank_gene$In )
write.csv(file = paste(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet23_GCI-_List_n197.csv', sep=''), x = rank_gene$De )
...
GFL <- cor.test(avg.cort.cere.express.df.nonan$Limbic, cere2cort.fcmri.nonan$Limbic)    # R = 0.36043915, p =  0.005043
GFC <- cor.test(avg.cort.cere.express.df.nonan$Cont , cere2cort.fcmri.nonan$Cont)       # R = -0.32691184, p = 0.0115
...
filename = paste(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet20_Gene_cortcere_Corr.csv', sep = '')
write.csv(x=avg.cort.cere.express.df.nonan, file = filename)
filename = paste(base.dir, '/Output/SupplementaryData/SUPPDATA_sheet21_FC_cortcere_Corr.csv', sep = '')
