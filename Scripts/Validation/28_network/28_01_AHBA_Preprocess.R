# -------------------------------
# -------------------------------
# Code to run AHBA pre-process in in lobular atlas:
# 
...
for ( donor in donor.nums ){
  ...
  all_data[[donor]]$cere_samples         <- all_data[[donor]]$raw_samp[cere_idxs,]                    
...
for ( donor in donor.nums ){
  # 28-network split component - Cere
  # --------------------
  atlas_28             <- paste(base.dir, '/Reference_files/MNI_Cerebellum28regions_2mm.nii.gz', sep = '')
  net28.assignments <- cere_query_10(data_struct=all_data[[donor]], atlas=atlas_28, MNI_coords=all_data[[donor]]$raw_samp, rad=0, afni.dir)
  write.csv(x=net28.assignments, file=paste(atlas_dir, '28net_', donor, '_cere.csv', sep='') )
}
...
# -------------------------------------------
for ( donor in donor.nums ){
  atlas_28   <- read.csv(paste(atlas_dir,'/28net_', donor, '_cere.csv', sep=''))
  all_data[[donor]][["Parcellation_28"]] <- atlas_28$x # store atlas assignments in the all_data structure
}
# -------------------------------------------
...
network_names[['twentyeight']] <- as.character(cere28_names_in$V2)
# -------------------------------------------
...
# -------------------------------------------
atlas_name <- 'Parcellation_28'
all_tables <- NULL
for ( donor in donor.nums ){
  cur_ref   <- network_names[['twentyeight']]
...
save(file=paste(data_path, '/all_data_cere_net28.Rdata', sep=''), x=all_data)
# load(file=paste(data_path, '/all_data_cere_28.Rdata', sep=''))
# -------------------------------------------
